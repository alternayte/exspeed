use exspeed_broker::replication::wire::{decode_event, encode_event, EncodedFrame};
...
fn encode_decode_records_appended_roundtrip() {
    ...
    let decoded = decode_event(opcode, &bytes).unwrap();
    match decoded {
...
#[test]
fn decode_event_with_wrong_opcode_errors() {
    let res = decode_event(OpCode::Ping, &[0xAB, 0xCD]);
    assert!(res.is_err(), "Ping is not a replication opcode");
}

#[test]
fn decode_garbage_payload_errors() {
    let res = decode_event(OpCode::StreamCreatedEvent, &[0xFF; 3]);
